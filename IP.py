...
def load_ip_data():
    if not os.path.exists('Indian_pines_corrected.mat') or not os.path.exists('Indian_pines_gt.mat'):
        raise FileNotFoundError(
            "Missing files! Please place 'Indian_pines_corrected.mat' and 'Indian_pines_gt.mat' in the script directory."
        )

    data = loadmat('Indian_pines_corrected.mat')['indian_pines_corrected']
    labels = loadmat('Indian_pines_gt.mat')['indian_pines_gt']

    X = data.reshape(-1, data.shape[2])  # shape: (145*145, 200)
    y = labels.reshape(-1)               # shape: (145*145,)
...
# ========== Model Definition ==========
def build_ip_model(input_dim, num_classes):
...
def train_and_evaluate(x_train, x_test, y_train, y_test, optimizer, label):
    model = build_ip_model(x_train.shape[1], y_train.shape[1])
...
# ========== Main ==========
def main():
    x_train, x_test, y_train, y_test = load_ip_data()
